...
- [ ] Lock. → The lock screen shows **Forgotten your passphrase?** under the Unlock button. It must not name a backup: the people with no backup are the ones who most need to open it.
- [ ] Open it. → Without scrolling, both answers are visible: a collapsed **I have no backup file** disclosure and **I have a backup file**, which the form below belongs to. The no-backup guidance must never be pushed below the form — those users will not scroll past four password fields to find it.
